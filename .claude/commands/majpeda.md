---
description: Met à jour [projet].peda et [projet].voca dans Notion pour la session en cours
...
Met à jour deux pages Notion pour le projet en cours : `[projet].peda` (journal pédagogique) et `[projet].voca` (vocabulaire).
...
## Étape 2 — Mise à jour de `[projet].voca`

### Rôle de la page `.voca`
...
### Ce qui relève de `.voca` (et non de `.peda`)

Un terme va dans `.voca` si :
...
Un concept va dans `.peda` (et pas seulement dans `.voca`) si :
...
**Les doublons sont acceptés et souhaitables.** Un terme peut avoir une entrée courte dans `.voca` ET une entrée longue dans `.peda`. Dans ce cas, ajouter dans `.voca` un lien hypertexte vers la section `.peda` correspondante.

### Structure d'une entrée `.voca`
...
2. Chercher la page `[projet].voca` dans Notion — la créer si elle n'existe pas (dans Notes & Docs, liée au projet)
