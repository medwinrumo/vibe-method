...
# pour réactivation si un futur build corrige l'affichage PreToolUse — voir ~/dev/wiki/methode-doc.md
...
case "$command" in
  *git\ commit*)
    echo "Doubt-driven : décision non triviale dans ce commit (archi, logique métier, périmètre inconnu) ? Si oui, cycle CLAIM -> EXTRACT -> DOUBT (sous-agent adversarial) -> RECONCILE -> STOP avant de valider. Voir ~/dev/wiki/methode-doc.md, section Pilotage de la session de code." >&2
    exit 1
    ;;
esac
exit 0
