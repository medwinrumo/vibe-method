...
**Skills sortis de la méthode le 05/08/2026 (phase 3).** `/lint`, `/wiki`, `/caveman`, `/pdf`, `/slides`, `/condense` et `/firecrawl` vivent désormais dans `~/dev/claude-config/commands/`. Ils restent invocables partout — seul le dépôt propriétaire change. Motif : leur objet n'est pas la méthode de développement (second cerveau, mode de communication, générateurs génériques). Critère de répartition dans `claude-config/README.md`.
